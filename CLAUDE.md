# CLAUDE.md — Parentlog Ai

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Parentlog Ai
- **Role**: AI-powered Parentlog companion
- **URL**: https://parentlog-ai.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/parentlog-ai
- **Branch**: master
...
### Deploy
```bash
cd /tmp/parentlog-ai && wrangler deploy
```

### Health Check
```bash
curl -s https://parentlog-ai.casey-digennaro.workers.dev/health
curl -s https://parentlog-ai.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
| /api/children | API endpoint |
| /api/memories | API endpoint |
| /api/milestones | API endpoint |
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=parentlog-ai
...
- **vessel.json capabilities**: parenting-tips, family-logging
...
1. Keep the vessel small. ~176 lines is the current size.
